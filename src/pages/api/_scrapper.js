...
const browserFetchUrl = process.env.HTMLFETCH_API
  ? `${process.env.HTMLFETCH_API}/?url=`
  : undefined;
...
const fetchCleanMarkdownFromUrl = async (
  ...
  try {
    ...
    let markdown = turndownService.turndown(
      removeNonContent ? `<h1>${article.title}</h1>${article.content}` : data
    );
    ...
    await processMarkdownWithImages(
      filePath,
      imgDirName,
    ...
    if (applyGpt) {
      ...
      const gptResponse = await runGPT(
        bigModel ? gptModelBig : gptModel,
        curMarkdown,
        instructions
      );
